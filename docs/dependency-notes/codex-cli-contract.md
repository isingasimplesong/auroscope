...
Local deterministic Go tests and vet pass; the documented fake-Paru disposable-Arch E2E also passes, all independently rerun by the delivery lane. The installed-package and supported-real-Paru gates were not run: both execute PKGBUILDs, explicitly forbidden in this task. The package source identity is intentionally unchanged pending completion of qualification and permission for the required disposable package gates. Authentication is resolved and must not be requested again. This candidate is not a delivered fix for the installed package.
