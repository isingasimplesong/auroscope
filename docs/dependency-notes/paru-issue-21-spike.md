...
The package originally moved from `paru>=2.1.0` to the concrete `paru-git` name. Issue #40 then exposed why that is the wrong package boundary: `makepkg --syncdeps` delegates an unsatisfied concrete name to Pacman, which cannot fetch an AUR package. The current AUR `paru-git` metadata instead advertises the unversioned virtual capability `provides=('paru')` and `conflicts=('paru')`.

The self-hosted AURoscope package therefore depends on the unversioned virtual `paru` capability. This lets an already installed compatible provider satisfy `makepkg`, while bootstrap instructions still install `paru-git` first because Pacman cannot obtain an absent AUR provider. A versioned virtual dependency is not usable while `paru-git` provides no version, and `paru --version` reports `2.1.0` for both the incompatible stable tag and tested post-fix builds. The package separately declares `conflicts=('paru<=2.1.0')`: Pacman can identify the known-broken stable package by its installed package version, while `paru-git`'s unversioned virtual provide does not falsely satisfy that versioned conflict. This prevents a packaged AURoscope installation from retaining stable Paru 2.1.0 and failing with an invisible selection menu. Compatibility beyond that known-broken release remains enforced by AURoscope's fail-closed selection/order parsers and the pinned supported-Paru E2E, not inferred from a provider package name or binary version string.
